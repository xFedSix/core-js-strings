...
function isPalindrome(str) {
  ...
  const revString = cleanedStr.split('').reverse().join('');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < splittedWords.length; i += 1) {
    if (splittedWords[i].length > longestWord.length) {
      longestWord = splittedWords[i];
    }
  }
  return longestWord;
}
...
function getStringFromTemplate(firstName, lastName) {
  // throw new Error('Not implemented');
  const joinedString = `${firstName} ${lastName}`;
  return `Hello, ${joinedString}!`;
}
...
function extractNameFromTemplate(value) {
  ...
  return splitedString[splitedString.length - 1].trimStart().slice(0, -1);
}
